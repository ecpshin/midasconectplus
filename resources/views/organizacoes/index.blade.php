<x-midas-layout>
    ...
    <div class="w-full">
        <div class="mx-auto w-full sm:px-4 lg:px-6">
            <div class="bg-white p-10 shadow-sm sm:rounded-lg">
                <a href="{{ route('admin.organizacoes.create') }}" class="rounded-lg bg-emerald-500 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-700">Adicionar</a>
                    ...
                    <tbody class="text-sm">
                        @foreach ($orgaos as $orgao)
                            <tr class="bg-gray-200 transition duration-200 even:bg-slate-50 hover:bg-red-400 hover:bg-opacity-20">
                                ...
                                <td class="py-1 pl-2 capitalize">
                                    {{ $orgao->nome_organizacao }}
                                </td>
                                <td class="flex items-center space-x-1 py-1 pl-2">
                                    <a href="{{ route('admin.organizacoes.show', $orgao) }}" class="rounded-md bg-sky-800 px-2 py-2 shadow-md shadow-slate-500 hover:bg-sky-500">
                                    ...
                                    <a href="{{ route('admin.organizacoes.edit', $orgao) }}"
                                    ...
                                    <a href="#" onclick="confirmar('{{ $orgao->id }}')"
                                        class="rounded-md bg-red-700 px-2 py-2 shadow-md shadow-slate-500 hover:bg-red-900">
                                    ...
                                    <form id="form_{{ $orgao->id }}" action="{{ route('admin.organizacoes.destroy', $orgao) }}" method="post">@csrf @method('DELETE')
                                    </form>
                                </td>
                            </tr>
                        @endforeach
                ...
                <div class="w-100 h-auto rounded-md bg-slate-300 px-3 py-1">
                    {{ $orgaos->links() }}
                </div>
