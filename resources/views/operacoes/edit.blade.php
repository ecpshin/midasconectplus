<x-midas-layout>
    <x-slot name="header">
        <x-bread page="{{ $page }}" area="{{ $area }}" rota="admin.operacoes.index"></x-bread>
    </x-slot>
        ...
        <div class="mx-auto w-full sm:px-4 lg:px-6">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-8 text-gray-900">
                    <form action="{{ route('admin.operacoes.update', $operacao) }}" class="mt-3" method="post">
                        @csrf @method('PATCH')
                        <div class="flex flex-col">
                            <label for="descricao_operacao" class="text-black">Descrição da Operação</label>
                            <input type="text" name="descricao_operacao" id="descricao_operacao" value="{{ old('descricao_operacao') ?? $operacao->descricao_operacao }}"
                                class="@error('descricao_operacao') border-red-500 @enderror mt-2 rounded-lg border-gray-300 py-1 focus:border-gray-300 focus:outline-none focus:ring-0">
                        ...
                        <a href="{{ route('admin.operacoes.index') }}"
